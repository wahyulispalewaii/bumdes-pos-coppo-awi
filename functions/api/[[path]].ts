...
async function login(ctx: RequestContext): Promise<Response> {
  ...
  if (user.locked_until && new Date(String(user.locked_until)).getTime() > Date.now()) {
    throw new HttpError(423, 'Akun terkunci sementara karena terlalu banyak percobaan login.');
  }
  ...
  await ctx.env.DB.batch([
    ctx.env.DB.prepare(`UPDATE users SET failed_login_count = 0, locked_until = NULL, last_login_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(user.id),
...
async function changePassword(ctx: RequestContext): Promise<Response> {
  ...
  if (newPassword.length < 8 || !/[A-Z]/.test(newPassword) || !/[a-z]/.test(newPassword) || !/\d/.test(newPassword)) {
    throw new HttpError(422, 'Password baru minimal 8 karakter dan mengandung huruf besar, huruf kecil, serta angka.');
  }
...
async function updateUser(ctx: RequestContext, userId: string): Promise<Response> {
  ...
  if (body.reset_password) {
    ...
    statements.push(ctx.env.DB.prepare(`UPDATE users SET password_hash=?,must_change_password=1,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(await hashPassword(password), userId));
