...
function toast(message, type = 'success', title = '') {
  ...
  node.innerHTML = `<div>${type === 'error' ? '⚠' : type === 'warning' ? '!' : '✓'}</div><div><strong>${escapeHtml(title || (type === 'error' ? 'Terjadi kesalahan' : 'Berhasil'))}</strong><p>${escapeHtml(message)}</p></div>`;
...
function handleError(error) {
  ...
  toast(error?.message || 'Terjadi kesalahan tidak dikenal.', 'error');
}
...
function openChangePassword() {
  modal({ title: 'Ganti Password Awal', subtitle: 'Untuk keamanan, ubah password bawaan sebelum melanjutkan penggunaan rutin.', body: `<form id="password-form" class="form-grid"><label class="field span-2"><span>Password saat ini</span><input name="current_password" type="password" required></label><label class="field"><span>Password baru</span><input name="new_password" type="password" required minlength="8"></label><label class="field"><span>Ulangi password baru</span><input name="confirm_password" type="password" required minlength="8"></label><div class="span-2 help-text">Minimal 8 karakter, mengandung huruf besar, huruf kecil, dan angka.</div></form>`, footer: `<button class="button outline" data-modal-close="button">Nanti</button><button id="change-password-submit" class="button primary">Ganti Password</button>` });
  document.querySelector('#change-password-submit').onclick=async(event)=>{const form=document.querySelector('#password-form');if(!form.reportValidity())return;const values=formObject(form);if(values.new_password!==values.confirm_password)return toast('Konfirmasi password tidak sama.','warning');const button=event.currentTarget;setButtonLoading(button,true);try{await post('auth/change-password',values);closeModal();toast('Password diperbarui. Silakan login kembali.');showLogin();}catch(error){handleError(error);setButtonLoading(button,false);}};
}
...
el.loginForm.addEventListener('submit', async (event) => {
  ...
  finally { setButtonLoading(el.loginButton, false); }
});

document.querySelector('#toggle-password').addEventListener('click', () => {
  const input = document.querySelector('#login-password'); input.type = input.type === 'password' ? 'text' : 'password';
});
document.querySelector('#logout-button').addEventListener('click', async () => { try { await post('auth/logout', {}); } catch {} showLogin(); toast('Anda telah keluar.'); });
